...
WHERE MONTH(payment_date)=5 AND YEAR(payment_date)=2005 AND DAY(payment_date)=27;
...
-- To use alphabetica order replace bottom line with:
-- ORDER BY CAST(rating AS CHAR) DESC;
...
SELECT payment_date, amount FROM 
(
...
SELECT title, description, special_features, length, rental_duration FROM 
(
	SELECT * FROM film
...
SELECT customer.first_name AS customer_first_name, customer.last_name AS customer_last_name, actor.first_name AS actor_first_name, actor.last_name AS actor_first_name
FROM customer
